...
// Note: In App Router, you don't need to disable bodyParser explicitly
// as you're handling the raw request directly with arrayBuffer()
  ...
  try {
    ...
    if (thumbnailFile) {
      ...
      if (!actualThumbnailPath) {
        return NextResponse.json({ error: "File path not found for thumbnail" }, { status: 500 })
      }
    ...
    if (avatarFile) {
      ...
      if (!actualAvatarPath) {
        return NextResponse.json({ error: "File path not found for creator avatar" }, { status: 500 })
      }
